  ...
  return (
    ...
    >
      <div className="max-w-[1280px] mx-auto px-6 flex items-center justify-between">
        <Link href="/" className="font-[var(--font-playfair)] text-2xl font-bold tracking-tight">
          Arjel Cosmeceuticals<span className="text-primary">.</span>
        </Link>
        ...
        <div className="flex items-center gap-4">
          ...
          {/* Account */}
          <Link href="/account" className="w-10 h-10 hidden sm:flex items-center justify-center rounded-full hover:bg-border transition-colors text-text-secondary hover:text-primary">
          ...
          {/* Cart */}
          <Link href="/cart" className="w-10 h-10 flex items-center justify-center rounded-full hover:bg-border transition-colors text-text-secondary hover:text-primary relative">
      ...
      {menuOpen && (
        <div className="lg:hidden absolute top-full left-0 right-0 bg-white/97 backdrop-blur-xl shadow-lg rounded-b-2xl p-6 flex flex-col gap-5 border-t border-border">
          {links.map((l) => (
            <Link key={l.href} href={l.href} onClick={() => setMenuOpen(false)} className="text-sm font-medium text-text-secondary hover:text-primary transition-colors">
              {l.label}
            </Link>
          ))}
        </div>
